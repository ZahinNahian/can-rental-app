        ...
        <div class="flex h-screen gap-8">
            ...
            <div class="flex-1 flex-col overflow-y-auto px-10">
                <div class="flex justify-end mb-5">
                    <a href="/admin/add-customer-page">
                        <button type="button"
                        ...
                        <div class="overflow-hidden">
                            <table class="min-w-full divide-y divide-neutral-200">
                                ...
                                <tbody class="divide-y divide-neutral-200">
                                    <tr class="text-neutral-800" v-for="(customer, index) in customers.data"
                                        :key="customer.id">
                                        <td class="px-5 py-4 text-sm font-medium whitespace-nowrap">{{ index + 1 }}</td>
                                        <td class="px-5 py-4 text-sm whitespace-nowrap">{{ customer.name }}</td>
                                        <td class="px-5 py-4 text-sm whitespace-nowrap">{{ customer.email }}</td>
                                        <td class="px-5 py-4 text-sm whitespace-nowrap">{{ customer.role }}</td>
                                        <td class="px-5 py-4 text-sm font-medium text-right whitespace-nowrap">
                                            <a class="text-blue-600 hover:text-blue-700"
                                                :href="`/admin/edit-customer-page/${customer.id}`">Edit</a> |
                                            <a class="text-blue-600 hover:text-blue-700"
                                                :href="`/admin/show-customer-page/${customer.id}`">Show</a> |
                                            <a class="text-blue-600 hover:text-blue-700"
                                                :href="`/admin/delete-customer/${customer.id}`">Delete</a>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
