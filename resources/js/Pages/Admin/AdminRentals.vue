            ...
            <div class="flex-1 flex-col overflow-y-auto px-10">
                            ...
                            <table class="min-w-full divide-y divide-neutral-200">
                                <thead>
                                    <tr class="text-neutral-500">
                                        ...
                                        <th class="px-5 py-3 text-xs font-medium text-left uppercase">Customer Name</th>
                                        <th class="px-5 py-3 text-xs font-medium text-left uppercase">Car Details(Name,Brand)</th>
                                ...
                                <tbody class="divide-y divide-neutral-200">
                                    <tr class="text-neutral-800" v-for="(rental, index) in rentals.data" :key="rental.id">
                                        <td class="px-5 py-4 text-sm font-medium whitespace-nowrap">{{ index+1 }}</td>
                                        <td class="px-5 py-4 text-sm whitespace-nowrap">{{ rental.user.name }}</td>
                                        <td class="px-5 py-4 text-sm whitespace-nowrap">{{ rental.car.name }}, {{ rental.car.brand }}</td>
                                        ...
                                        <td class="px-5 py-4 text-sm font-medium text-right whitespace-nowrap">
                                            <a class="text-blue-600 hover:text-blue-700" :href="`/admin/edit-rental-page/${rental.id}`">Edit</a> |
                ...
                <div class="flex items-center justify-between w-full h-16 px-3 border-t border-neutral-200">
                    <p class="pl-2 text-sm text-gray-700">
                        Showing
                        <span class="font-medium">{{ rentals.from }}</span>
                        to
                        <span class="font-medium">{{ rentals.to }}</span>
                        of
                        <span class="font-medium">{{ rentals.total }}</span> results
                    </p>

                    <nav v-if="rentals.links.length > 3">
                        <ul
                            ...
                            <li v-for="(link, index) in rentals.links" :key="index" class="h-full">
